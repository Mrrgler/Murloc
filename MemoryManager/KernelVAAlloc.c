...
void KernelVAAllocInit(void)
{
	...
	mrgl_tree_insert(&kernel_va_alloc_header.pAddrHeader, convert_addr_to(KERNEL_VA_TOP_BASE), &pBlock->AddrNode);
	mrgl_sizelist_insert(&kernel_va_alloc_header.pSizeHeader, &pBlock->SizeNode);
}
...
addr_t KernelVAAlloc(uint32_t pages_num)
{
	...
	pNode = mrgl_sizelist_find(&kernel_va_alloc_header.pSizeHeader, pages_num);
	if(pNode == NULL){
	...
	mrgl_tree_remove(&kernel_va_alloc_header.pAddrHeader, &pBlock->AddrNode);
	mrgl_sizelist_remove(&kernel_va_alloc_header.pSizeHeader, &pBlock->SizeNode);
	...
	if(pBlock->SizeNode.size > pages_num){
		...
		mrgl_tree_insert(&kernel_va_alloc_header.pAddrHeader, pBlock->AddrNode.key + pages_num, &pBlock->AddrNode);
		mrgl_sizelist_insert(&kernel_va_alloc_header.pSizeHeader, &pBlock->SizeNode);
	}else{
...
uint32_t KernelVAFree(addr_t addr, uint32_t pages_num)
{
	...
	pNode = mrgl_tree_find(&kernel_va_alloc_header.pAddrHeader, convert_addr_to(addr));
	...
	if(pNode == NULL){
		...
		mrgl_tree_insert(&kernel_va_alloc_header.pAddrHeader, convert_addr_to(addr), &pBlock->AddrNode);
		mrgl_sizelist_insert(&kernel_va_alloc_header.pSizeHeader, &pBlock->SizeNode);
	}else{
